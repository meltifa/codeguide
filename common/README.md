...
附Dreamweaver代码格式的设置，需要整理html的时候选择[命令]-[应用源格式]即可。点击[高级格式设置]右侧的CSS按钮可进入CSS源格式的设置
...
当使用特定厂商的带有前缀的属性时，最后一行必须有不带前缀的标准CSS3属性，有些现代浏览器只支持不带前缀的CSS3属性，如IE10 & 11。
...
对于外部独立的js或css文件，在使用中文注释时要保证文件和引用页面的编码完全一致，否则IE6下会出现代码无法解析的BUG！
...
* 一条普遍规则，不要添加不必要的约束。（例如ul#someid {...} .menu#otherid{...}）
